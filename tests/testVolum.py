...
def main():
    ...
    if server is None:
        raise RuntimeError("无法创建服务器，请确保正确安装 trame 及其依赖")
    print("Server:", server)
    print("Server state:", server.state)

    # 初始化数据源
    dicom_dir = "data/81344"  # 替换为你的 DICOM 目录路径
    try:
        metadata = parse_dicom_metadata(dicom_dir)
        print(f"DICOM 元数据: {metadata[:1]}")
    except Exception as e:
        print(f"解析 DICOM 元数据失败: {e}")
        return

    # 初始化可视化器并绑定 UI
    visualizer = VTKVolumeVisualizer(server, dicom_dir)
    visualizer.bind_ui()

    # 定义状态变更回调
    @server.state.change("reset_camera")
    def reset_camera(**kwargs):
        print("触发重置相机")
        visualizer.reset_camera()

    @server.state.change("update_opacity")
    def update_opacity(opacity_scale, **kwargs):
        print(f"调整不透明度缩放: {opacity_scale}")
        volumes = visualizer.renderer.GetVolumes()
        if volumes.GetNumberOfItems() > 0:
            volume = volumes.GetItemAsObject(0)
            # 由于 volume 是 vtkVolume 类型，需先安全转换为 vtkVolume，再获取属性
            ...
            if isinstance(volume, vtkVolume):
                ...
                print("警告: 获取到的对象不是 vtkVolume，无法调整不透明度")
                return
            ...
            if visualizer.vtk_view:
                visualizer.vtk_view.update()
                visualizer.render_window.Render()
    ...
    # 绑定交互事件
    if hasattr(server.state, "on_change") and callable(server.state.on_change):
        server.state.on_change("*", update_interaction)
    else:
        print("警告: server.state.on_change 不可用，交互事件未绑定")

    # 启动服务器
    server.start(port=8080, open_browser=False, disable_logging=False)
